...
class Grid(object):
    ...
    def __init__(self, report, labels, threshold=150):
        ...
        self.frequency_labels = [label for label in labels if label.is_frequency()]
        self.threshold_labels = [label for label in labels if label.is_threshold()]
        ...
        x_lines = [label.find_closest_line(lines) for label in self.frequency_labels]
        x_pixels = [line[0].get_x() for line in x_lines]
        x_frequency = [label.get_value() for label in self.frequency_labels]
        self.x_distances = [line[1] for line in x_lines]

        y_lines = [label.find_closest_line(lines) for label in self.threshold_labels]
        y_pixels = [line[0].get_y() for line in y_lines]
        y_threshold = [label.get_value() for label in self.threshold_labels]
    ...
    def _find_closest_label(self, value, labels):
        closest_label = min(labels, key=lambda label: abs(label.get_value() - value))
        return closest_label, closest_label.label_confidence
    ...
    def get_snapped_frequency(self, symbol: Symbol, epsilon: float = 0.15) -> tuple:
        ...
        """
        frequency = self.pixel_freq_map(symbol.get_center()["x"])

        if symbol.conduction == "air":
          snapped_freq = Audiology.round_frequency(frequency)
        else:
            snapped_freq = Audiology.round_frequency_bone(frequency, symbol.ear, epsilon=epsilon)
        
        closest_label, confidence = self._find_closest_label(snapped_freq, self.frequency_labels)
        return snapped_freq, confidence
    ...
    def get_snapped_threshold(self, symbol: Symbol) -> tuple:
        ...
        """
        threshold = self.pixel_threshold_map(symbol.get_center()["y"])
        snapped_threshold = Audiology.round_threshold(threshold)
        
        closest_label, confidence = self._find_closest_label(snapped_threshold, self.threshold_labels)
        return snapped_threshold, confidence
