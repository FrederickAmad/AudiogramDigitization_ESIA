...
def calculate_review_needed(symbol_conf, freq_conf, thresh_conf, symbol_weight=0.05, \
                            freq_weight=0.455, thresh_weight=0.455, threshold=0.70):
    """
    Allows for the user to adjust the weighting given to the confidences
    
    Returns
    -------
    Boolean (True/False)
    """
    weighted_avg = (symbol_weight * symbol_conf + freq_weight * freq_conf + thresh_weight * thresh_conf) \
                    / (symbol_weight + freq_weight + thresh_weight)
        ...
        thresholds += [
          {
            ...
            "symbol confidence": round(symbol.symbol_confidence * 100, 2),
            "frequency": snapped_freq,
            "frequency confidence": round(freq_confidence * 100, 2),
            "threshold": snapped_threshold,
            "threshold confidence": round(threshold_confidence * 100, 2),
            # The 'calculate_review_needed' function allows for the adjustment of
            # how much weighting to give the confidence of each component
            "REVIEW NEEDED?": calculate_review_needed(symbol.symbol_confidence, freq_confidence, threshold_confidence)
            }
            for symbol in symbols
            for snapped_freq, freq_confidence in [grid.get_snapped_frequency(symbol)]
            for snapped_threshold, threshold_confidence in [grid.get_snapped_threshold(symbol)]
        ]
